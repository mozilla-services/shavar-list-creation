...
def publish_to_remote_settings(config, section, chunknum, version):
    ...
    filter_exp = f'env.version <= "{LOWEST_VERSION_SUPPORTED}a1"'
    id_exp = list_name

    if version is not None:
        # the version format has to be of the form {versionnum}a1 => eg. 129.0a1
        filter_exp = f'env.version == "{version}a1"'
...
# what should the default version be?
